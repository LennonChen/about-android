...
struct typestring : public qtype
{
  ...
  bool is_funcptr(void) const
  {
    if ( empty() )
      return false;
    const type_t *t2 = begin();
    if ( !is_type_ptr(*t2) )
      return false;
    t2 = skip_ptr_type_header(t2);
    return is_type_func(*t2);
  }
  ...
  bool is_const(void) const    { const type_t *ptr = resolve(); return ptr != NULL && is_type_const(*ptr); }
...
typestring hexapi dummy_ptrtype(int ptrsize, bool isfp);
...
typestring hexapi make_array(const type_t *type, int nelems);
...
class vdloc_t : public varloc_t
{
  int regoff(void); // inaccessible & undefined: regoff() should not be used
public:
  // use all available bits for register number for VLOC_REG1
  int reg1(void) const { return type() == VLOC_REG2 ? varloc_t::reg1() : stkoff(); }
  void _set_reg1(int r1) { varloc_t::_set_reg1(r1, 0); } // it works fine
  void set_reg1(int r1) { cleanup_varloc(this); _set_reg1(r1); }
};

//-------------------------------------------------------------------------
/// Do two arglocs overlap?
bool hexapi arglocs_overlap(vdloc_t loc1, size_t w1, vdloc_t loc2, size_t w2);
...
struct lvar_locator_t
{
  ...
  const scattered_vloc_t &get_scattered(void) const { return location.scattered(); }
        scattered_vloc_t &get_scattered(void)       { return location.scattered(); }
...
public:
  ...
  lvar_t(const qstring &n, const vdloc_t &l, ea_t e, const typestring &t, int w, int db)
...
struct lvars_t : public qvector<lvar_t>
{
  ...
  /// \return -1 if failed, otherwise the index into the variables vector.
  int hexapi find_lvar(vdloc_t location, int width, int defblk=-1);
};
...
cexpr_t *hexapi make_num(uint64 n, cfunc_t *func=NULL, ea_t ea=BADADDR, int opnum=0, type_sign_t sign=no_sign);
...
qstring hexapi create_field_name(const typestring &type, uval_t offset=BADADDR);
typestring hexapi dummy_plist_for(const type_t *ptr);
typestring hexapi make_dt(int n);
typedef void *hexdsp_t(int code, ...);
const int HEXRAYS_API_MAGIC1 = 0xC001C0DE;
const int HEXRAYS_API_MAGIC2 = 0xBE39BABE; // xxNNxxxx
...
//@}
  ...
  history_t old_history;        // obsolete
  location_t *ploc;             // use ida6.3 location object
  ...
  void hexapi switch_to(cfuncptr_t f);
  ...
  /// \return false if failed
  bool __fastcall hexapi pop_current_location(int omflags=0);
  ...
  bool __fastcall hexapi set_vargloc_end(ea_t ea, varloc_t ida_argloc);
...
//--------------------------------------------------------------------------
inline typestring dummy_ptrtype(int ptrsize, bool isfp)
{
  typestring retval;
...
//--------------------------------------------------------------------------
inline typestring make_array(const type_t *type, int nelems)
{
  typestring retval;
...
//--------------------------------------------------------------------------
inline bool strtype_info_t::create_from(const type_t *type, const p_list *fields, bool with_gaps)
{
  return (bool)(size_t)hexdsp(hx_strtype_info_t_create_from, this, type, fields, with_gaps);
}
...
//--------------------------------------------------------------------------
inline bool arglocs_overlap(vdloc_t loc1, size_t w1, vdloc_t loc2, size_t w2)
{
  int32 loc1_raw = loc1.get_value(); // varloc_t is passed by value
  int32 loc2_raw = loc2.get_value(); // varloc_t is passed by value
  return (bool)(size_t)hexdsp(hx_arglocs_overlap, loc1_raw, w1, loc2_raw, w2);
}
...
//--------------------------------------------------------------------------
inline int lvars_t::find_lvar(vdloc_t location, int width, int defblk)
{
  int32 location_raw = location.get_value(); // varloc_t is passed by value
  return (int)hexdsp(hx_lvars_t_find_lvar, this, location_raw, width, defblk);
}
...
{
  return (bool)(size_t)hexdsp(hx_decompile_many, outfile, funcaddrs, flags);
}
...
//--------------------------------------------------------------------------
inline cexpr_t *make_num(uint64 n, cfunc_t *func, ea_t ea, int opnum, type_sign_t sign)
{
  return (cexpr_t *)hexdsp(hx_make_num, &n, func, ea, opnum, sign);
}
...
{
  return (bool)(size_t)hexdsp(hx_cfunc_t_get_user_union_selection, this, ea, path);
}
...
{
  return (bool)(size_t)hexdsp(hx_mark_cfunc_dirty, ea);
}
...
{
  return (bool)(size_t)hexdsp(hx_has_cached_cfunc, ea);
}
...
//--------------------------------------------------------------------------
inline qstring create_field_name(const typestring &type, uval_t offset)
{
  qstring retval;
...
//--------------------------------------------------------------------------
inline typestring dummy_plist_for(const type_t *ptr)
{
  typestring retval;
  hexdsp(hx_dummy_plist_for, &retval, ptr);
  return retval;
}

//--------------------------------------------------------------------------
inline typestring make_dt(int n)
{
  typestring retval;
  hexdsp(hx_make_dt, &retval, n);
  return retval;
}
...
//--------------------------------------------------------------------------
inline void vdui_t::switch_to(cfuncptr_t f)
{
...
{
  return (bool)(size_t)hexdsp(hx_vdui_t_ui_map_lvar, this, v);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::ui_unmap_lvar(lvar_t *v)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_ui_unmap_lvar, this, v);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::map_lvar(lvar_t *from, lvar_t *to)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_map_lvar, this, from, to);
}
...
{
  return (bool)(size_t)hexdsp(hx_vdui_t_jump_enter, this, idv, omflags);
}
...
//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::pop_current_location(int omflags)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_pop_current_location, this, omflags);
}
...
{
  return (bool)(size_t)hexdsp(hx_vdui_t_collapse_lvars, this, hide);
}
...
//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::set_vargloc_end(ea_t ea, varloc_t ida_argloc)
{
  int32 ida_argloc_raw = ida_argloc.get_value(); // varloc_t is passed by value
  return (bool)(size_t)hexdsp(hx_vdui_t_set_vargloc_end, this, ea, ida_argloc_raw);
}
